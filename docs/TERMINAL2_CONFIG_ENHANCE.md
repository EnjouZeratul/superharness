...
- [x] 新增环境变量读取
  - [x] `CONTINUUM_PROVIDER`
  - [x] `CONTINUUM_API_KEY`
  - [x] `CONTINUUM_BASE_URL`
  - [x] `CONTINUUM_MODEL`
- [x] 新增多提供商管理
...
- [x] 新增子命令
  ...
  continuum config add-provider <name> --key <key> --url <url> --model <model>
  continuum config use <provider>    # 切换提供商
  continuum config show              # 显示当前配置
  continuum config list              # 列出所有提供商
  continuum config set <key> <value> # 设置配置项
  continuum config get <key>         # 获取配置项
  ```
- [x] 预计时间: 2小时
- **完成时间**: 2026-05-11

### T2.3: 配置文件默认路径 ✅
- [x] 默认路径: `~/.continuum/config.toml`
- [x] 支持项目级覆盖: `.continuum/config.toml`
- [x] 首次运行自动创建默认配置
- [x] 预计时间: 1小时
- **完成时间**: 2026-05-11
...
### 新增功能:
- 环境变量支持: `CONTINUUM_PROVIDER`, `CONTINUUM_API_KEY`, etc.
- 环境变量引用: `${VAR_NAME}` 语法
- 多提供商管理: 添加/切换/列出提供商
- 配置优先级: 环境变量 > 配置文件 > 默认值

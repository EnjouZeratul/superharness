//! # SuperHarness Core
//!
//! 统一重导出所有 layer 模块，并提供 Python 绑定。
...
fn sh_core(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    // Layer 0
    m.add_class::<py_bindings::PySecurityGateway>()?;

    // Layer 1
    m.add_class::<py_bindings::PyLlmClient>()?;
    m.add_class::<py_bindings::PyCostTracker>()?;

    // Layer 2
    m.add_class::<py_bindings::PyAgentRuntime>()?;
    m.add_class::<py_bindings::PySessionManager>()?;
    m.add_class::<py_bindings::PyCheckpointSystem>()?;
    m.add_class::<py_bindings::PyAgent>()?;
    m.add_class::<py_bindings::PySession>()?;

    // Layer 3
    m.add_class::<py_bindings::PyToolExecutor>()?;
    m.add_class::<py_bindings::PyQueryEngine>()?;
    m.add_class::<py_bindings::PyMemorySystem>()?;

    // Layer 4
    m.add_class::<py_bindings::PyMcpBridge>()?;
    m.add_class::<py_bindings::PyAuditLogger>()?;
